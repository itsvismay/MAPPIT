...
namespace crowds
{
	void reg_gradient(VectorXd& q, int num_agents, int num_points_per_agent, VectorXd& K, VectorXd& g)
	{	
		g.resize(q.size());
		g.setZero();
		...
		for(int i=0; i<num_agents; i++)
		{
			...
			for(s =0; s<num_points_per_agent - 1; s++)
			{
				...
				g(i*3*num_points_per_agent + 3*s+2) += K(i)*kt/(dt*dt);
				g(i*3*num_points_per_agent + 3*(s+1)+2) += -K(i)*kt/(dt*dt);

			}

			//Technically the last element should be 0 since 'kt' is not exactly a constant
