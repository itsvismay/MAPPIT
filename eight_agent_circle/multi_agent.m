...
%options.HessianFcn = 'steepestdesc';
q = fmincon(@(x) path_energy(x, numel(scene.agents), e, surf_anim), reshape(v', numel(v),1), A,b,Aeq,beq,[],[], @(x)constraints(x, scene), options);
...
function p = SamplePointsForRod(x)
    ts = linspace(x(1,3)+0.001, x(end,3)-0.001, 50); %time slots to sample
