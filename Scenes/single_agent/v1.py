...
def make_space_time_rod(x, y, t):
	for i in range(0, len(t)-1):
		cylinder_between(x[i], y[i], t[i], x[i+1], y[i+1], t[i+1], float(rod["thickness"]))


f = open("single_agent.json", "r")
rod = json.loads(f.read())
rod["path"] = np.array(rod["path"])
...
imported_object = bpy.ops.import_scene.obj(filepath="roomba.obj", split_mode='OFF')
